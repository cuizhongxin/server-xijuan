...
public class BattleCalculator {
    ...
    public static DamageResult calcDamage(BattleUnit attacker, BattleUnit target) {
        ...
        double yanhangBonus = 1.0;
        double defendReduceMul = 1.0;
        ...
        if ("t_infantry_3".equals(attacker.tacticsId) && attacker.tacticsLevel > 0 && target.troopType == 3) {
            yanhangBonus = 1.0 + calcYanhangArcherBonus(attacker.tacticsLevel) / 100.0;
            finalDamage *= yanhangBonus;
        }
        ...
        if (target.tacticsId != null && finalDamage > 0) {
            switch (target.tacticsId) {
                case "t_infantry_1": // 方圆阵 - 全减伤
                    defendReduceMul = Math.max(0.3, 1.0 - target.tacticsEffectValue / 100.0);
                    finalDamage *= defendReduceMul;
                    break;
                case "t_infantry_3": // 雁行阵 - 减骑兵伤害
                    if (attacker.troopType == 2) {
                        defendReduceMul = Math.max(0.2, 1.0 - target.tacticsEffectValue / 100.0);
                        finalDamage *= defendReduceMul;
                    }
                    break;
